...
export const MarqueeTicker: React.FC<MarqueeTickerProps> = ({ announcements, recentCompletedPlate }) => {
  ...
  const tickerText =
    activeAnnouncements.length > 0
      ...
      : 'PRECISION AUTO-WORKSHOP TELEMETRY SYSTEM • HIGH PERFORMANCE DIAGNOSTICS & REPAIR TRACKING';

  return (
    <div className="bg-[#15171e] border-t border-white/10 text-white flex items-center h-14 overflow-hidden px-6 relative shadow-2xl z-30">
      ...
      {recentCompletedPlate ? (
        <div className="absolute inset-0 bg-gradient-to-r from-emerald-600 via-teal-500 to-emerald-600 text-slate-950 font-black flex items-center justify-center px-4 z-20 animate-pulse font-mono">
          ...
          <span className="text-sm tracking-wider uppercase">
            🎉 ATTENTION: CAR PLATE <span className="bg-slate-950 text-emerald-400 px-2.5 py-1 rounded ml-1 mr-1">{recentCompletedPlate.plate}</span> FOR {recentCompletedPlate.owner.toUpperCase()} IS READY FOR PICKUP!
          </span>
        </div>
      ) : null}
